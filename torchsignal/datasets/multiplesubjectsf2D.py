...
class MultipleSubjectsf():
    ...
    def leave_one_subject_out(self, selected_subject_id=1, dataloader_batchsize=32, dataloader_shuffle=False):
       ...
       # data loader
       train_loader1 = DataLoader(train_dataset, batch_size=dataloader_batchsize, shuffle=dataloader_shuffle)
       val_loader1 = DataLoader(val_dataset, batch_size=dataloader_batchsize, shuffle=dataloader_shuffle)
       selected_subject_loader1 = DataLoader(test_dataset, batch_size=dataloader_batchsize, shuffle=False)
       
       return train_loader1,val_loader1, selected_subject_loader1
...
def _load_multiple(root, dataset: PyTorchDataset, subject_ids: [], sessions: [], verbose: bool = False) -> None:
    ...
    for subject_id in subject_ids:
        print('Frequency Load subject:', subject_id)
        subject_data = None
        subject_target = None
...
def _process_data(data_by_subjects, selected_channels, segment_config, bandpass_config):
    ...
    for subject_id in list(data_by_subjects.keys()):
        ...
        #window=hamming(1000)
        n=4096
        yf=abs(scipy.fft.rfft(subject_data,n))
        yf=20*np.log10(yf)
        subject_data=yf[:,:,100:1100]
